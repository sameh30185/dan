...
$func_parse_vc_tta_section = function( $matches ) use( &$us_tabs_atts, &$active_tab_indexes ) {
	...
	// TODO: replace do_shortcode with other condition to check and remove $us_grid_layouts_backup patch
	global $us_grid_layouts;
	$us_grid_layouts_backup = $us_grid_layouts;
	if ( empty( $matches[5] ) OR empty( do_shortcode( $matches[5] ) ) ) {
		return;
	}
	$us_grid_layouts = $us_grid_layouts_backup;
	unset( $us_grid_layouts_backup );
