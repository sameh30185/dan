		...
		foreach ( $current_acf_filters as $acf_field_name => $acf_item ) {
			if ( function_exists( 'acf_get_field' ) ) {
				...
				foreach ( array_keys( us_arr_path( $acf_field, 'choices', array() ) ) as $item ) {
					$item_key = preg_replace( '/\s/', '_', mb_strtolower( $item ) );
	...
	function us_search_grid_filter_the_post( $post_id ) {
		...
		if ( $post = get_post( (int) $post_id ) ) {
			...
			if ( strpos( $post->post_content, $substring ) !== FALSE ) {
				...
				us_get_recursive_parse_page_block( $post, function( $post ) use ( &$result ) {
					if (
						is_null( $result )
						AND $post instanceof WP_Post
	...
	function us_save_post_first_grid_data( $post_id ) {
		...
		$func_search_first_grid = function( $post ) {
			if (
				...
				$post_type = 'post';
				if ( ! empty( $atts['post_type'] ) ) {
					if ( is_array( $atts['post_type'] ) AND ! empty( $atts['post_type'][0] ) ) {
						$post_type = $atts['post_type'][0];
					} else {
						$post_type = $atts['post_type'];
					}
				}
				update_post_meta( $post->ID, '_us_first_grid_post_type', $post_type );
				...
				if ( in_array( $post_type, array_keys( us_grid_available_taxonomies() ) ) ) {
...
if ( ! function_exists( 'us_get_grid_filter_url_prefix' ) ) {
	/**
	 * Get prefix for Grid Filters
	 *
	 * @return string
	 */
	function us_get_grid_filter_url_prefix() {
		$prefix = us_get_option( 'grid_filter_url_prefix', 'filter' );
		if ( empty( $prefix ) ) {
			$prefix = 'filter';
		}
		return $prefix;
